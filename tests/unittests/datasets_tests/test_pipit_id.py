...
@pytest.fixture
def dataset() -> Dataset:
    return PipitId(split="train_within_year")


@pytest.fixture
def dataset_with_transforms() -> Dataset:
    ds = PipitId(split="train_within_year")
    cfg = DatasetConfig(
...
def dataset_with_output_mapping() -> Dataset:
    mapping = {"individual_id": "pipit_id"}
    return PipitId(split="test_within_year", output_take_and_give=mapping)
...
def test_output_mapping(dataset_with_output_mapping: Dataset) -> None:
    ...
    assert set(sample.keys()) == {"pipit_id"}
