...
STAGE="${HOME}/esp-data-staging/fasd13_support"
SCRIPT="scripts/data_preprocessing_scripts/fasd13/build_fasd13.py"

export CLOUDSDK_CONFIG="$(mktemp -d)"
export GCE_METADATA_MTLS_MODE=none
export ESP_DATA_GCS_REQUESTS_TIMEOUT=600

rm -rf "${STAGE}"; mkdir -p "${STAGE}/raw" "${STAGE}/manifests" "${STAGE}/out"
echo "=== fetch annotations + manifests ==="
gsutil -m -q cp -r "${GCS}/raw/*" "${STAGE}/raw/"
...
uv run python "${SCRIPT}" support \
    --root "${STAGE}/raw" --mirrors "${GCS}" --out-root "${STAGE}/out" \
